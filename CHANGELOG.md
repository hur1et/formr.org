...
### Added
- 2026-03-13: "Download AI Chats (JSON)" in the "Export Run Data" menu of each run.
  Downloads all `ai_chat` item responses as a structured JSON file with `session` (same
  identifier as in CSV/XLSX exports), `item`, `answered`, and `conversation` (array of
  `{role, content}` turns).

### Changed
- 2026-03-13: Refactored AI backend internals for maintainability; no behaviour changes.
  Input validation for the run-participant AI endpoint is now consistent with
  the researcher API endpoint (message structure, prompt length).

### Fixed
- 2026-03-13: AI chat widget now shows specific error messages for rate limits (429),
  service unavailability (503), and network failures instead of a generic fallback.
